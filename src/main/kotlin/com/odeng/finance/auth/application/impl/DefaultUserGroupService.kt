...
class DefaultUserGroupService(private val userGroupRepository: UserGroupRepository) : UserGroupService {
    ...
    override fun share(input: UserGroupShareInput) {
        logger.info { "Sharing userGroup: ${input.userGroupId} with userId: ${input.sharedWithUserId}" }
        ...
        // Business Rule 3: Check if user already has access (duplicate check)
        ...
        userGroupRepository.add(input.userGroupId, input.sharedWithUserId)
    }
